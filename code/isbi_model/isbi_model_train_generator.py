...
# Helper Functions
# Points translations function
def translate_points(point,translation): 
    point = point + translation 
    
    return point

# Points rotation function
...
class Generator(object):
    def __init__(self,
                 ...
                 rotate_ratio=0.1,
                 contrast_ratio=0.1,
                 flip_indices=[(0,34),(1,35),(2,36),(3,37),(4,38),(5,39),\
                 (6,40),(7,41),(8,42),(9,43),(10,44),(11,45),(12,46),(13,47),\
                 (14,48),(15,49),(16,50),(17,51),(18,52),(19,53),(20,54),\
                 (21,55),(22,56),(23,57),(24,58),(25,59),(26,60),(27,61),\
                 (28,62),(29,63),(30,64),(31,65),(32,66),(33,67),(68,68),\
                 (69,69),(70,72),(71,73)]
                 ):
        """
        Arguments
    ...
    def translation(self): 
        ...
        for i in range(np.shape(mask)[0]): 
             mask[i,:,:,0] = cv2.warpAffine(mask[i,:,:,0],\
             np.float32([[1,0,tx],[0,1,ty]]),(512,384))
        ...
        for i in range(np.shape(image)[0]):
            for j in range(3):
                image[i,:,:,j] = cv2.warpAffine(image[i,:,:,j],\
                np.float32([[1,0,tx],[0,1,ty]]),(512,384))
    ...
    def rotate(self):
        ...
        for i in indices: 
            for j in range(3): 
                self.inputs[i,:,:,j] = \
                cv2.warpAffine(self.inputs[i,:,:,j], M, (512,384))
        ...
        for i in indices: 
            ...
            x_r[j], y_r[j] = \
            rotate_points((256/512,192/512),(x_r[j],y_r[j]),(-angle * 2 * np.pi)/360)
    ...
    def generate(self, batchsize=32): 
        ...
        while True:
            ...
            for start, end in cuts:
                ...
                # Need this to avoid indices out of bounds
                self.flip()
                self.translation()
                self.rotate()
                
                yield (self.inputs, {'conv2d_19':self.mask, 'conv2d_38':self.mask,\
                'conv2d_57':self.mask, 'keypoints':self.targets})